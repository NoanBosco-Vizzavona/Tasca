# -*- coding: utf-8 -*-
"""train_test_dz_dziribert_15%_augmented

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1uHMn3ti2f0Fqxu6o5tAWiG6BaKxXAFOr
"""
...
df_10p = pd.read_csv('/content/df_train_with_catt_DZ1_15p.tsv', sep='\t')

# Fusionner les text_catt depuis df_full dans df_10p là où tweet est identique
df_fixed = df_10p.copy()

# Créer un dictionnaire tweet ➜ text_catt depuis l'original complet
...
# Sauvegarder le dataset réparé
df_fixed.to_csv('df_train_augmented_complete_tashkeel.tsv', sep='\t', index=False)

# Statistiques post-fusion
print("✅ Total avec Tashkeel maintenant :", df_fixed['text_catt'].notna().sum(), "/", len(df_fixed))
...
# Sauvegarder les datasets modifiés sans NaN
...
print("✅ Nettoyage terminé ! Les nouveaux fichiers sans NaN sont sauvegardés sous :")
print(f"- {train_clean_path}")
print(f"- {test_clean_path}")
print(f"- {val_clean_path}")

# Afficher un aperçu des fichiers nettoyé
...
print("Total (3 datasets combinés):")
print(total_counts)
...
# Fonction pour équilibrer un dataframe à N exemples par label
...
# Sauvegarder les nouveaux datasets équilibrés
...
# Afficher la répartition finale pour confirmation
...
checkpoint_path = "./checkpoints_epochs/model_seed_66_epoch_8.pth"  # Chemin du modèle
...
# Charger le modèle fine-tuné
checkpoint_path = "./checkpoints_epochs/model_seed_66_epoch_1_catt.pth"  # Chemin du modèle
# Use the correct model architecture: AutoModelForSequenceClassification
# and the same pre-trained model used for training
model = BertForSequenceClassification.from_pretrained("alger-ia/dziribert_sentiment")
model.load_state_dict(torch.load(checkpoint_path))  # Charger les poids entraînés
